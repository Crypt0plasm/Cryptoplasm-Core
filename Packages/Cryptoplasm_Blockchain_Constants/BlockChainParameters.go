...
const (
    ...
    CryptoplasmStdMathPrecision     = uint32(70)                //Total Precision
...
var (
    ...
    CryptoplasmPrecisionContext 	= p.Context{
        Precision:   		        CryptoplasmStdMathPrecision,
    ...
    //
    //=============================================================================================
    ngSeed03 			            = SUBs(p.NFI(0), Seed03)
    ngSeed09 			            = SUBs(p.NFI(0), Seed09)
    ngSeed29 			            = SUBs(p.NFI(0), Seed29)
    //
...
func ConvertIntervals(interval *p.Decimal) (*p.Decimal, *p.Decimal, *p.Decimal, *p.Decimal) {
    ...
    Years := DIVs(interval, p.NFI(525600))
    _, _ = c.Quantize(pYears, Years, 0)
    Rest := SUBs(interval, MULs(pYears, p.NFI(525600)))

    Days := DIVs(Rest, p.NFI(1440))
    _, _ = c.Quantize(pDays, Days, 0)
    Rest = SUBs(Rest, MULs(pDays, p.NFI(1440)))

    Hours := DIVs(Rest, p.NFI(60))
    _, _ = c.Quantize(pHours, Hours, 0)
    Rest = SUBs(Rest, MULs(pHours, p.NFI(60)))
