...
func TxSimulator (BlockHeight,TransactionSize,OutputNumber uint64, cpAmount *firefly.Decimal) {
	...
	Difference1 := SUBpr(IP,cpAmount,Ten)
	IsThreshold1 := Difference1.IsZero()
	Difference2 := SUBpr(IP,cpAmount,One)
	...
	if IsThreshold1 == true || Difference1.Negative == false {
		...
		OverSend := CPOverSend(cpAmount)
		NormalFee := Fees[0][0][0]
		BlinkFee  := Fees[1][0][0]
		fmt.Println("")
		fmt.Println("For the Recipient to receive",cpAmount,"CP")
		...
		BlinkFee  := Fees[1][1][0]
		fmt.Println("Recipient will receive",cpAmount,"CP")
