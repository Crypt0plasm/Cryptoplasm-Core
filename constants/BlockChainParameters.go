...
var (
	...
	//=============================================================================================
	CryptoplasmPrecisionContext = precision.Context{
		Precision:   CryptoplasmContextPrecision,
		MaxExponent: precision.MaxExponent,
		MinExponent: precision.MinExponent,
		Rounding:    precision.RoundDown,
		// Default error conditions.
		Traps: precision.InvalidOperation,
	}
	//
	//
	// Blockchain Premine and Emission variable Declarations
	//=============================================================================================
	TotalEmission, _, _ = precision.NewFromString(TheoreticalEraEmission)
	PreMine, _, _       = precision.NewFromString(PreMinedAmount)
	StartBRd, _, _      = precision.NewFromString(TheoreticalStartBlockReward)
	AUs, _, _           = precision.NewFromString(AuPerUnit)
	...
	//
	FpBMax, _, _ = precision.NewFromString(MaxFeePerByte)
	FpBInt, _, _ = precision.NewFromString(FeePerByteDecrement)
	FpBMin, _, _ = precision.NewFromString(MinFeePerByte)
	FpBThr, _, _ = precision.NewFromString(FeePerByteBHThreshold)
	//===============================
	//
	// Intervals and Seeds variable Declaration
	//=============================================================================================
	Purple, _, _ = precision.NewFromString(IntervalPurple)
	Indigo, _, _ = precision.NewFromString(IntervalIndigo)
	Blue, _, _   = precision.NewFromString(IntervalBlue)
	Green, _, _  = precision.NewFromString(IntervalGreen)
	Yellow, _, _ = precision.NewFromString(IntervalYellow)
	Orange, _, _ = precision.NewFromString(IntervalOrange)
	Red, _, _    = precision.NewFromString(IntervalRed)
	White, _, _  = precision.NewFromString(IntervalWhite)
	//===============================
	Seed1st, _, _ = precision.NewFromString(BaseAreaSeed)
	Seed2nd, _, _ = precision.NewFromString(BaseHeightSeed)
	Seed3rd, _, _ = precision.NewFromString(CPAreaRatioSeed)
	Seed4th, _, _ = precision.NewFromString(TotalComputedSum)
	Seed5th, _, _ = precision.NewFromString(HighestComputedBR)
	//===============================
	Seed01, _, _ = precision.NewFromString(PurpleUpHeightSeed)
	Seed02, _, _ = precision.NewFromString(PurpleUpAreaSeed)
	Seed03, _, _ = precision.NewFromString(PurpleDownHeightSeed)
	Seed04, _, _ = precision.NewFromString(PurpleDownAreaSeed)
	//===============================
	Seed05, _, _ = precision.NewFromString(IndigoUpHeightPeakSeed)
	Seed06, _, _ = precision.NewFromString(IndigoUpAreaPeakSeed)
	Seed07, _, _ = precision.NewFromString(IndigoDownHeightPeakSeed)
	Seed08, _, _ = precision.NewFromString(IndigoDownAreaPeakSeed)
	//===============================
	Seed09, _, _ = precision.NewFromString(BlueUpHeightPeakSeed)
	Seed10, _, _ = precision.NewFromString(BlueUpAreaPeakSeed)
	Seed11, _, _ = precision.NewFromString(BlueDownHeightPeakSeed)
	Seed12, _, _ = precision.NewFromString(BlueDownAreaPeakSeed)
	//===============================
	Seed13, _, _ = precision.NewFromString(GreenUpHeightPeakSeed)
	Seed14, _, _ = precision.NewFromString(GreenUpAreaPeakSeed)
	Seed15, _, _ = precision.NewFromString(GreenDownHeightPeakSeed)
	Seed16, _, _ = precision.NewFromString(GreenDownAreaPeakSeed)
	//===============================
	Seed17, _, _ = precision.NewFromString(YellowUpHeightPeakSeed)
	Seed18, _, _ = precision.NewFromString(YellowUpAreaPeakSeed)
	Seed19, _, _ = precision.NewFromString(YellowDownHeightPeakSeed)
	Seed20, _, _ = precision.NewFromString(YellowDownAreaPeakSeed)
	//===============================
	Seed21, _, _ = precision.NewFromString(OrangeUpHeightPeakSeed)
	Seed22, _, _ = precision.NewFromString(OrangeUpAreaPeakSeed)
	Seed23, _, _ = precision.NewFromString(OrangeDownHeightPeakSeed)
	Seed24, _, _ = precision.NewFromString(OrangeDownAreaPeakSeed)
	//===============================
	Seed25, _, _ = precision.NewFromString(RedUpHeightPeakSeed)
	Seed26, _, _ = precision.NewFromString(RedUpAreaPeakSeed)
	Seed27, _, _ = precision.NewFromString(RedDownHeightPeakSeed)
	Seed28, _, _ = precision.NewFromString(RedDownAreaPeakSeed)
	//===============================
	Seed29, _, _ = precision.NewFromString(WhiteHeightPeakSeed)
	Seed30, _, _ = precision.NewFromString(WhiteAreaPeakSeed)
	//===============================
	...
	//
	//=============================================================================================
	ngSeed03 = SUBcp(precision.NFI(0), Seed03)
	ngSeed09 = SUBcp(precision.NFI(0), Seed09)
	ngSeed29 = SUBcp(precision.NFI(0), Seed29)
	//
...
func ConvertIntervals(interval *precision.Decimal) (*precision.Decimal, *precision.Decimal, *precision.Decimal, *precision.Decimal) {
	var (
		pHours = new(precision.Decimal)
		pDays  = new(precision.Decimal)
		pYears = new(precision.Decimal)
	)

	Years := DIVcp(interval, precision.NFI(525600))
	_, _ = c.Quantize(pYears, Years, 0)
	Rest := SUBcp(interval, MULcp(pYears, precision.NFI(525600)))

	Days := DIVcp(Rest, precision.NFI(1440))
	_, _ = c.Quantize(pDays, Days, 0)
	Rest = SUBcp(Rest, MULcp(pDays, precision.NFI(1440)))

	Hours := DIVcp(Rest, precision.NFI(60))
	_, _ = c.Quantize(pHours, Hours, 0)
	Rest = SUBcp(Rest, MULcp(pHours, precision.NFI(60)))
